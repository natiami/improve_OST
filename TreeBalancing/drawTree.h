...
void my_free(void * p) {
	allocs_in_use--; free(p);

}

void * my_alloc(int size) {
	void * p = malloc(size); allocs_in_use++;
...
void free_ptree(Pnode *pn) {
	...
	if (pn == NULL) return;
	free_ptree(pn->left);
	free_ptree(pn->right); my_free(pn);

}

Pnode * build_ptree_rec(Node<int> * t, Node<int> *end)
{ 
	Pnode * pn;
	...
	sprintf_s(pn->label, "(%d)", t->key, t->N);
...
Pnode * build_ptree(Node<int> * t, Node<int> *end) {
...
void compute_edge_lengths(Pnode *pn) {
	...
	/* now fill in the height of pn */ h = 1;
	if (pn->left != NULL) {
...
void pretty_print_tree(Node<int> * t, Node<int> * end) {
	Pnode *proot; int xmin, i;
	if (t == end) return;
